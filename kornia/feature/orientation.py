...
class PatchDominantGradientOrientation(nn.Module):
    ...
    def forward(self, patch: torch.Tensor) -> torch.Tensor:
        ...
        smooth_weight = self.angular_smooth.weight.to(device=ang_bins.device, dtype=ang_bins.dtype)
        ang_bins = F.conv1d(F.pad(ang_bins, (2, 2), mode="circular"), smooth_weight).view(-1, self.num_ang_bins)
